<template x-if="$store.modal.activeModal == 'CreateCalendar'">
    <form
        id="calendarCreateForm"
        x-show="$store.modal.activeModal == 'CreateCalendar'"
        x-transition.opacity
        x-data="{
            formData: $store.modal.modalsData['CalendarModal']['CreateCalendar'],
            send: async function() {
                this.sending = true;
                const payload = Object.fromEntries(
                    new FormData(this.$el),
                );
                const res = await $store.axios.post('/calendars', payload);
                if (res.success) {
    ...
    >
        ...
        <div class="mt-8 flex justify-between gap-4">
            ...
            <x-modal.buttons.send-form
                class="basis-1/3"
                ::disabled="!$validate.isComplete('calendarCreateForm') || sending"
            />
        </div>
    </form>
</template>
